---
title: "Suprisal Analysis of Umm-Containing Sentences (Validation)"
author: "Lauren Oey"
date: "12/5/2018"
output: pdf_document
---
...
df <- read_csv("umm_kenlm_output_validation.csv")
glimpse(df)
```
...
df.final %>%
  ...
  ggplot(aes(x=lexicalType, y=avgPreSplitScoreByLen)) +
  geom_violin() +
  ggtitle("Avg Pre-Umm Surprisal by Umm vs Controlby Number of Words Before Umm") +
...
multipleLexLengths <- df.final %>%
  ...
  mutate(sum = sum(count)) %>%
  filter(lexicalType == "umm" & count != sum) %>%
  select(author, lexicalType, sentLength, lexicalLength) %>%
